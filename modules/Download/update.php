...
// updates file info
$row = new CMS_Download( (int) InputData::read('id') );
$row['name'] = $download['name'];
$row['content'] = $download['content'];
$row['binary'] = $download['binary'];

// blob
if($row['binary'])
{
    if($_FILES['download']['size']['file'] > 0)
    {
