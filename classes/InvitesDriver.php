...
class InvitesDriver implements IOTS_GuildAction
{
    ...
    public function listRequests()
    {
        ...
        foreach( $this->db->query('SELECT `name` FROM [invites] WHERE `content` = ' . $this->guild->getId() ) as $invite)
        {
    ...
    {
        $this->db->query('INSERT INTO [invites] (`name`, `content`) VALUES (' . $player->getId() . ', ' . $this->guild->getId() . ')');
    }

    // un-invites player
    public function deleteRequest(OTS_Player $player)
    {
        $this->db->query('DELETE FROM [invites] WHERE `name` = ' . $player->getId() . ' AND `content` = ' . $this->guild->getId() );
    }
