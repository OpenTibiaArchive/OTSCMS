...
class CMS_Invite extends ActiveRecord
{
    // loads record by given ID
    public function load($id)
    {
        $load = $this->db->prepare('SELECT `id`, `name`, `content` FROM [invites] WHERE `id` = :id');
    ...
    public function save()
    {
        ...
        {
            $save = $this->db->prepare('UPDATE [invites] SET `name` = :name, `content` = :content WHERE `id` = :id');
            $save->execute( array(':name' => $this->data['name'], ':content' => $this->data['content'], ':id' => $this->data['id']) );
        }
        // if no then inserts it as new row
        else
        {
            $save = $this->db->prepare('INSERT INTO [invites] (`name`, `content`) VALUES (:name, :content)');
