...
class CMS_Request extends ActiveRecord
{
    // loads record by given ID
    public function load($id)
    {
        $load = $this->db->prepare('SELECT `id`, `name`, `content` FROM [requests] WHERE `id` = :id');
    ...
    public function save()
    {
        ...
        {
            $save = $this->db->prepare('UPDATE [requests] SET `name` = :name, `content` = :content WHERE `id` = :id');
            $save->execute( array(':name' => $this->data['name'], ':content' => $this->data['content'], ':id' => $this->data['id']) );
        }
        // if no then inserts it as new row
        else
        {
            $save = $this->db->prepare('INSERT INTO [requests] (`name`, `content`) VALUES (:name, :content)');
