...
class CMS_Download extends ActiveRecord
{
    // loads record by given ID
    public function load($id)
    {
        $load = $this->db->prepare('SELECT `id`, `name`, `content`, `binary`, `file` FROM [download] WHERE `id` = :id');
    ...
    public function save()
    {
        ...
        {
            $save = $this->db->prepare('UPDATE [download] SET `name` = :name, `content` = :content, `binary` = :binary, `file` = :file WHERE `id` = :id');
        ...
        {
            $save = $this->db->prepare('INSERT INTO [download] (`name`, `content`, `binary`, `file`) VALUES (:name, :content, :binary, :file)');
