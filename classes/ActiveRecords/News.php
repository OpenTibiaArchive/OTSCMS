...
class CMS_News extends ActiveRecord
{
    // loads record by given ID
    public function load($id)
    {
        $load = $this->db->prepare('SELECT `id`, `name`, `content`, `date_time` FROM [news] WHERE `id` = :id');
    ...
    public function save()
    {
        ...
        {
            $save = $this->db->prepare('UPDATE [news] SET `name` = :name, `content` = :content, `date_time` = :date_time WHERE `id` = :id');
        ...
        {
            $save = $this->db->prepare('INSERT INTO [news] (`name`, `content`, `date_time`) VALUES (:name, :content, :date_time)');
